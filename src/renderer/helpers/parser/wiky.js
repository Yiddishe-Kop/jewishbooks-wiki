...
var Wiky = {
  ...
  rules: {
    ...
    pre: [
      ...
      { rex: /([\u05do-\u05ea])"([\u05do-\u05ea])/g, tmplt: '$1״$2' }, // replace quotes inside Hebrew words with Gershayim
    ],
    post: [
      ...
      { rex: /\[[01AIacdgilprtu]+[:]|[:][01AIacdgilprtu]+\]/g, tmplt: '' }, // .. remove singular section delimiters (they frequently exist with incomplete documents while typing) ..
      {
        ...
        tmplt: function($0, $1, $2, $3, $4) {
          return (
            ...
            ($1 ? ' colspan="' + $1 + '"' : '') +
            ($2 == ' '
              ? ' style="text-align:' + ($2 == $4 ? 'center' : 'right') + ';"'
              : $4 == ' '
    ...
    wikiinlines: [
      ...
      { rex: /\?([^ \t\f\v\xB6]+)\((.+)\)\?/g, tmplt: '<abbr title="$2">$1</abbr>' }, // .. abbreviation ..
      {
        ...
        tmplt: function($0, $1, $2, $3) {
          return Wiky.store(
            '<img' + Wiky.style($1) + ' src="' + $2 + '" alt="' + ($3 ? $3 : $2) + '" title="' + ($3 ? $3 : $2) + '"/>'
          );
        },
    ...
    stender: [
      { rex: /\'{3}(.+?)\'{3}/g, tmplt: '<strong>$1</strong>' }, // bold
      {
        rex: /\[{2}(.+?)\|(.+?)\]{2}/g,
        tmplt: function($0, $1, $2) {
          return $2 == '*' ? `<a href="${$1}">${$1}</a>` : `<a href="${$1}">${$2}</a>`;
        },
      }, // links with text
      { rex: /\{{2}מרכז\|(.+?)\}{2}\n/g, tmplt: '<center>$1</center>' }, // מרכז
      { rex: /\[{2}(.+?)(\|\*)?\]{2}/g, tmplt: '<a href="$1">$1</a>' }, // links
      { rex: /\{{2}ממ\|([^\}]+)\}{2}/g, tmplt: '<small>($1)</small>' }, // מראי מקומות
      { rex: /\{{2}הערה\|([^\}]+)\}{2}/g, tmplt: '<sup>*<span class="fn-popup">$1</span></sup>' }, // הערות
      {
        rex: /\{{2}גופן\|(\d)\|{2}(.+?)\}{2}/g,
        tmplt: function($0, $1, $2) {
          let fontSize = 0.5 * $1;
          return `<span style="font-size:${fontSize}em">${$2}</span>`;
        },
      }, // גופן
    ],
    hide: [
      {
        rex: /\{{2}(הועלה אוטומטית|ניווט כללי עליון|ניווט כללי תחתון|פורסם בנחלת הכלל|שולי הגליון|הערות שוליים|ocr)\}{2}/g,
        tmplt: '',
      },
    ],
    lang: {},
  ...
  inverse: {
    ...
    nonwikiblocks: [
      {
        ...
        tmplt: function($0, $1, $2) {
          return Wiky.store(
            '[' +
              Wiky.invStyle($1) +
              Wiky.invAttr($1, ['lang']).replace(/x\-/, '') +
              '%' +
              Wiky.apply(
                $2,
                Wiky.hasAttr($1, 'lang') ? Wiky.inverse.lang[Wiky.attrVal($1, 'lang').substr(2)] : Wiky.inverse.code
              ) +
              '%]'
          );
        },
    ...
    wikiblocks: [
      ...
      { rex: /<h1>(.*?)<\/h1>/gim, tmplt: '=$1=' },
      { rex: /<h2>(.*?)<\/h2>/gim, tmplt: '==$1==' },
      { rex: /<h3>(.*?)<\/h3>/gim, tmplt: '===$1===' },
      { rex: /<h4>(.*?)<\/h4>/gim, tmplt: '====$1====' },
      { rex: /<h5>(.*?)<\/h5>/gim, tmplt: '=====$1=====' },
      { rex: /<h6>(.*?)<\/h6>/gim, tmplt: '======$1======' },
      {
        rex: /<(p|table)[^>]+(style=\"[^\"]*\")[^>]*>/gim,
        tmplt: function($0, $1, $2) {
          return '<' + $1 + '>' + Wiky.invStyle($2);
        },
      },
      { rex: /\xB6{2}<li/gim, tmplt: '\xB6<li' }, // ie6 only ..
      {
        rex: /<li class=\"?([^ >\"]*)\"?[^>]*?>([^<]*)/gim,
        tmplt: function($0, $1, $2) {
      ...
      { rex: /(^|\xB6)<(u|o)l[^>]*?>\xB6/gim, tmplt: '$1' }, // only outer level list start at BOL ...
      ...
      { rex: /<\/blockquote>/gim, tmplt: '"]' },
      { rex: /<td class=\"?lft\"?>\xB6*[ ]?|<\/tr>/gim, tmplt: '|' }, // ie6 only ..
      { rex: /\xB6<tr(?:[^>]*?)>/gim, tmplt: '\xB6' },
      { rex: /<td colspan=\"([0-9]+)\"(?:[^>]*?)>/gim, tmplt: '|$1>' },
      { rex: /<td(?:[^>]*?)>/gim, tmplt: '|' },
      { rex: /<table>/gim, tmplt: '[' },
      { rex: /<\/table>/gim, tmplt: ']' },
      { rex: /<tr(?:[^>]*?)>\xB6*|<\/td>\xB6*|<tbody>\xB6*|<\/tbody>/gim, tmplt: '' },
      { rex: /<hr\/?>/gim, tmplt: '----' },
      { rex: /<br\/?>/gim, tmplt: '\\\\' },
      { rex: /(<p>|<(d|o|u)l[^>]*>|<\/(dl|ol|ul|p)>|<\/(li|dd)>)/gim, tmplt: '' },
    ...
    wikiinlines: [
      { rex: /<strong[^>]*?>(.*?)<\/strong>/gim, tmplt: '*$1*' },
      { rex: /<b[^>]*?>(.*?)<\/b>/gim, tmplt: '*$1*' },
      { rex: /<em[^>]*?>(.*?)<\/em>/gim, tmplt: '_$1_' },
      { rex: /<i[^>]*?>(.*?)<\/i>/gim, tmplt: '_$1_' },
      { rex: /<sup[^>]*?>(.*?)<\/sup>/gim, tmplt: '^$1^' },
      { rex: /<sub[^>]*?>(.*?)<\/sub>/gim, tmplt: '~$1~' },
      { rex: /<del[^>]*?>(.*?)<\/del>/gim, tmplt: '(-$1-)' },
      { rex: /<abbr title=\"([^\"]*)\">(.*?)<\/abbr>/gim, tmplt: '?$2($1)?' },
      {
        rex: /<a href=\"([^\"]*)\"[^>]*?>(.*?)<\/a>/gim,
        tmplt: function($0, $1, $2) {
          return $1 == $2 ? $1 : '[' + $1 + ',' + $2 + ']';
  ...
  apply: function(str, rules) {
    if (str && rules)
      for (var i in rules) {
        if (typeof rules[i] == 'string') str = Wiky.apply(str, eval(rules[i]));
        else str = str.replace(rules[i].rex, rules[i].tmplt);
      }
    return str;
  },
  store: function(str, unresolved) {
    return unresolved
      ? '@' + (Wiky.blocks.push(str) - 1) + '@'
      : '@' +
          (Wiky.blocks.push(
            str.replace(/@([0-9]+)@/g, function($0, $1) {
              return Wiky.restore($1);
            })
          ) -
            1) +
  ...
  invAttr: function(str, names) {
    var a = [],
      x;
    for (var i in names)
      if (str.indexOf(names[i] + '=') >= 0) a.push(str.replace(new RegExp('^.*?' + names[i] + '="(.*?)".*?$'), '$1'));
  ...
  sectionRule: function(fromLevel, style, content, toLevel) {
    ...
    return !trf[transition]
      ? '?(' + transition + ')' // error string !
      : trf[transition]
          .replace(/\$2/, ' class="' + fromLevel + '"')
