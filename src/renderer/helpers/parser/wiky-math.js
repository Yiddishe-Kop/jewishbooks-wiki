...
Wiky.rules.math = {
  ...
  expr: [
    ...
    }, // fraction
    {
      rex: /\[((\[(([^,\]]+[,]){1,}[^\]]+)\][ \n]*){1,})\]/g,
      tmplt: function($0, $1) {
        var m = Wiky.math.transpose(
            $1
              .replace(/(^\[|\]$)/g, '')
              .replace(/(\][ \n]*\[)/g, '|')
              .split('|')
          ),
          sz = ' style="font-size:' + m.len + '00%;"';
        /*alert("{("+m.mat.join(")}\n{(").split(",").join(")(")+")}");*/ return (
          '<span class="lb"' +
          sz +
          '>' +
          Wiky.math.fence() +
          '</span><span class="m"><span class="e">' +
          m.mat
            .join('</span></span>\n<span class="m"><span class="e">')
            .split(',')
            .join('</span><span class="e">') +
          '</span></span><span class="rb"' +
          sz +
          '>' +
          Wiky.math.fence() +
          '</span>'
    ...
    }, // matrix
    {
      ...
      tmplt: function($0, $1) {
        var v = $1.split(','),
          sz = ' style="font-size:' + v.length + '00%;"';
        return (
          '<span class="lb"' +
          sz +
          '>' +
          Wiky.math.fence() +
          '</span><span class="v"><span class="e">' +
          v.join('</span><span class="e">') +
          '</span></span><span class="rb"' +
          sz +
          '>' +
          Wiky.math.fence() +
...
Wiky.inverse.math = {
  ...
  expr: [
    ...
    {
      rex: /<span class=\"o\"><span>@[0-9]+@<\/span>(\\prod|\\sum|\\int)<span class=\"x\">(\{?@[0-9]+@\}?)<\/span><\/span>/gim,
      tmplt: '$1_$2',
      dbg: true,
    }, // underscript
    {
      rex: /<span class=\"o\"><span class=\"x\">(\{?@[0-9]+@\}?)<\/span>(\\prod|\\sum|\\int)<span>@[0-9]+@<\/span><\/span>/gim,
      tmplt: '$2^$1',
    }, // overscript
    {
      rex: /<span class=\"f\"><span class=\"n\">(\{?@[0-9]+@\}?)<\/span><span class="d">(\{?@[0-9]+@\}?)<\/span><\/span>/gim,
      tmplt: '$1/$2',
    }, // fraction
    {
      rex: /<span class=\"lb\"[^>]*>&[^;]+;<\/span><span class=\"v\">((?:<span class=\"e\">[^>]*<\/span>){2,})<\/span><span class=\"rb\"[^>]*>&[^;]+;<\/span>/gim,
      tmplt: function($0, $1) {
        return (
          '[' + $1.replace(/(?:^<span class=\"e\">|<\/span>$)/g, '').replace(/<\/span><span class=\"e\">/g, ',') + ']'
        );
      },
    }, // vector ..
    {
      rex: /<span class=\"lb\"[^>]*>&[^;]+;<\/span>((?:<span class=\"m\">(?:(?:<span class=\"e\">[^>]*<\/span>){2,})<\/span>[^>]*){2,})<span class=\"rb\"[^>]*>&[^;]+;<\/span>/gim,
      tmplt: function($0, $1) {
        return (
          '[[' +
          Wiky.math
            .transpose(
              $1
                .replace(/(?:^<span class=\"m\"><span class=\"e\">|<\/span><\/span>$)/g, '')
                .replace(/<\/span><span class=\"e\">/g, ',')
                .replace(/<\/span><\/span>[^>]*<span class=\"m\"><span class=\"e\">/g, '|')
                .split('|')
            )
            .mat.join('][') +
          ']]'
    ...
    { rex: /<span class=\"b\">(@[0-9]+@)<\/span>/gim, tmplt: '!$1' }, // bold vector ..
    { rex: /<sup>(\{?@[0-9]+@\}?)<\/sup>&#8260;<sub>(\{?@[0-9]+@\}?)<\/sub>/gim, tmplt: '$1//$2' },
    { rex: /<sup class=\"i\">(\{?@[0-9]+@\}?)<\/sup>/gim, tmplt: '^$1' },
    { rex: /<sub class=\"i\">(\{?@[0-9]+@\}?)<\/sub>/gim, tmplt: '_$1' },
  ],
  symbols: [
    // greek symbols
...
Wiky.math = {
  toHtml: function(str) {
    var expr = function(itr) {
      // region from "{" to "}", nesting allowed ..
      var s = '';
...
Wiky.inverse.nonwikiblocks = Wiky.inverse.nonwikiblocks.concat([
  {
    rex: /<div class=\"eq\"><a name=\"eq([0-9]+)\">(?:.*?)<\/a>(.*?)<\/div>/g,
    tmplt: function($0, $1, $2) {
      return Wiky.store('[(' + $1 + ')$' + Wiky.math.toWiki($2) + '$]');
    },
  }, // numbered equation
  {
    rex: /<div class=\"eq\">(.*?)<\/div>/g,
    tmplt: function($0, $1) {
